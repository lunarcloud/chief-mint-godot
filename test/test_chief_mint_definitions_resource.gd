...
func test_is_class_identifies_correctly():
	var definitions = ChiefMintDefinitionsResource.new()
	assert_true(
		definitions.is_class("ChiefMintDefinitionsResource"),
		"Should identify as ChiefMintDefinitionsResource"
	)
...
func test_can_store_definitions_array():
	...
	assert_eq(
		definitions.definitions[0].name, "Achievement 1", "Should store first definition correctly"
	)
	assert_eq(
		definitions.definitions[1].name, "Achievement 2", "Should store second definition correctly"
	)
